...
def get_llm(model, cache_dir="llm_weights", max_len=sys.maxsize):
    ...
    model.seqlen = min(model.config.max_position_embeddings, max_len)
    return model
...
########################################### for train
    ...
    parser.add_argument(
        "--no_prune_zeros", action="store_true", help="do not consider zeros as pruning target")
    ...
    model = get_llm(args.model, args.cache_dir, args.max_len)
    ...
    ############################ owl   ############################
        ...
        prune_wanda_outlier(args, model, tokenizer, device, prune_n=prune_n, prune_m=prune_m, no_prune_zeros=args.no_prune_zeros)
    ...
    if args.save_model:
        ...
        with open(os.path.join(args.save_model, f"sparsity_{sparsity_ratio}"), "w") as f:
            f.write(f"{sparsity_ratio}")
