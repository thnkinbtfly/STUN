...
def check_outlier_mean(mask, threshold, non_zeros_mask=None):
    W = mask
    if non_zeros_mask is not None:
        W = W[non_zeros_mask]
...
@torch.no_grad()
def prune_wanda(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, nsamples=128):
    use_cache = model.config.use_cache
    model.config.use_cache = False
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            print(f"pruning layer {i} name {name} {wrapped_layers[name].nsamples}")
            if not wrapped_layers[name].nsamples:
                print(f"NO SAMPLES FOR {name}. CONTINUING!!!!!!")
                continue
...
@torch.no_grad()
def prune_wanda_outlier(args, model, tokenizer, device=torch.device("cuda:0"), prune_n=0, prune_m=0, nsamples=128,
                        no_prune_zeros=False):
    ##### calucalte outlier ratio
    ...
    nonzero_params = collections.defaultdict(int)
    for i in range(len(layers)):
        layer = layers[i].to(device)
        ...
        for name in subset:
            if no_prune_zeros:
                cur_non_zeros_mask = (subset[name].weight.data != 0).flatten().cpu()
                if non_zeros_mask is None:
                    non_zeros_mask = cur_non_zeros_mask
                    nonzero_params[i] += cur_non_zeros_mask.sum().item()
                else:
                    non_zeros_mask = torch.cat((non_zeros_mask, cur_non_zeros_mask))
            ...
            W_metric = torch.abs(subset[name].weight.data) * torch.sqrt(
                wrapped_layers[name].scaler_row.reshape((1, -1)))
            W_metric = W_metric.cpu()

            # activation_data = torch.sqrt(wrapped_layers[name].scaler_row.reshape((1, -1))).cpu()
            layer_wmetric.append(W_metric)
            wrapped_layers[name].free()
        ...
        for out_ratio in [args.Hyper_m]:
            out_ratio_layer = check_outlier_mean(layer_wmetric, out_ratio, non_zeros_mask=non_zeros_mask)
            print("layer outlier ratio", out_ratio, out_ratio_layer)
    ...
    total_sums = 0
    # if no_prune_zeros:
    #     for i, layer_ratio in enumerate(all_layer_ratio):
    #         coeffs += nonzero_params[i]
    #         total_sums += nonzero_params[i] * (1 - args.sparsity_ratio - layer_ratio)
    #     all_layer_ratio += (total_sums / coeffs)
    # else:
    all_layer_ratio += (1 - args.sparsity_ratio)
    ...
    for i in range(len(layers)):
        ...
        for name in subset:
            ...
            if prune_n != 0:
                ...
                # if no_prune_zeros:
                #     zeros_mask = (subset[name].weight.data == 0)
                #     W_metric[zeros_mask] = W_metric.max() + 1 # avoid pruning
                sort_res = torch.sort(W_metric, dim=-1, stable=True)
                ...
                if args.use_variant:
                    ...
                    # unstructured pruning
                    sort_res_indices = sort_res[1]
                    if no_prune_zeros:
                        whether_zeros = subset[name].weight.data.abs().sum(0)
                        zero_rows = (whether_zeros == 0).sum().item()
                        zero_row_ratios = zero_rows / len(whether_zeros)
                        layer_sparsity_ratio /= (1 - zero_row_ratios)
                        print(layer_sparsity_ratio)
                    mask_nums = int(W_metric.shape[1] * layer_sparsity_ratio)

                    # if no_prune_zeros:
                    #     zeros_mask = (subset[name].weight.data == 0)
                    #     for mask_i, (index, zeros) in enumerate(zip(sort_res_indices, zeros_mask)):
                    #         indices = sort_res_indices[:mask_nums + zeros.sum().item()]
                    #         W_mask[mask_i][indices] = True
                    # else:
                    indices = sort_res_indices[:, :mask_nums]
                    W_mask.scatter_(1, indices, True)
            subset[name].weight.data[W_mask] = 0  ## set weights to zero
            wrapped_layers[name].free()
