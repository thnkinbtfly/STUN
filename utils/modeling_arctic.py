...
def load_balancing_loss_func(
        gate_logits: torch.Tensor, num_experts: torch.Tensor = None, top_k=4,
    ...
    _, selected_experts = torch.topk(routing_weights, min(top_k, concatenated_gate_logits.shape[1]), dim=-1)
...
def get_arctic_linear(input_dim,
    ...
    if is_deepspeed_available():
        ...
        return ds_linear.OptimizedLinear(input_dim, output_dim, bias, ds_optimized_lora_config,
                                         ds_optimized_quantization_config, dtype=dtype)
...
class ArcticMoE(nn.Module):
    def __init__(self, config: ArcticConfig, layer_id: int, num_experts: int, **kwargs):
        ...
        if not self.is_moe_layer:  # dense, not MoE
            ...
            if self.use_deepspeed_implementation:  # DeepSpeed's MoE
                ...
                self.experts = nn.ModuleList([ArcticMLP(config,
                                                        ...
                                                        ds_optimized_quantization_config=quantization_config,
                                                        ds_optimized_lora_config=deepspeed_lora,
                                                        shard_base_weights_if_doing_lora=True) for i in
    ...
    def _moe_foreward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        routing_weights, selected_experts = torch.topk(routing_weights, min(self.top_k, router_logits.shape[1]), dim=-1)
        if self.top_k > 1:
        ...
        return final_hidden_states, load_balancing_loss_func((router_logits,), self.num_experts,
                                                             self.top_k)  # ZY: let's directly output the loss to align what we have in ds
...
class ArcticDecoderLayer(nn.Module):
    def __init__(self, config: ArcticConfig, layer_idx: int, **kwargs):
        ...
        self.block_sparse_moe = ArcticMoE(config, layer_id=layer_idx, num_experts=config.num_local_experts[layer_idx], **kwargs)
...
class ArcticModel(ArcticPreTrainedModel):
    ...
    def forward(
        ...
        next_cache = None
        if use_cache:
            next_cache = next_decoder_cache.to_legacy_cache() if use_legacy_cache and hasattr(next_decoder_cache,
                                                                                              'to_legacy_cache') else next_decoder_cache
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if self.is_deepspeed_lora:
            for param_name in list(
                    state_dict.keys()):  # Use list to avoid RuntimeError due to changing size during iteration
                if param_name.endswith("base_weight"):
                    ...
                    if self.shard_base_weights_if_doing_lora and 'deepspeed_moe.experts.deepspeed_experts' not in param_name:
                        gathered_weights = [torch.zeros_like(base_weight,
                                                             device=base_weight.device, dtype=base_weight.dtype) for _
                                            in range(world_size)]
                        torch.distributed.gather(base_weight, gather_list=gathered_weights if rank == 0 else None,
                                                 dst=0, group=None)
        ...
        if self.is_deepspeed_lora:
            ...
            for param_name in local_state_dict:
                ...
                if 'deepspeed_moe' in incoming_param_name:
                    ...
                    assert shape_incoming[1] == shape_local[
                        1] * world_size, "weights should be sharded equally across world size"
